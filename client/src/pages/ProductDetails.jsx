...
function ProductDetails() {
  ...
  useEffect(() => {
    const localProduct = products.find((item) => item._id === id);
    if (localProduct) {
      setProduct(localProduct);
    } else {
      // Step 2: Fallback API call if not found in context
      axios
        .get(`https://zestmart-grocery.onrender.com/api/product/${id}`)
        .then((res) => {
        ...
        <div className="flex gap-3">
          <div className="flex flex-col gap-3">
            {product.image.map((singleimage, index) => (
              <div
                key={index}
                onClick={() => setThumbnail(singleimage)}
                className="border max-w-24 border-gray-500/30 rounded overflow-hidden cursor-pointer"
              >
                <img src={singleimage} alt={`Thumbnail ${index + 1}`} />
              </div>
            ))}
          </div>
        ...
        <div className="text-sm w-full md:w-1/2">
          ...
          <div className="mt-6">
            ...
            <p className="text-2xl font-medium">
              Offer: {currency} {product.offerPrice}
            </p>
